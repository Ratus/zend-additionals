    ...
    AdapterAwareInterface
{
    ...
    protected $entityColumnMapping = array();
    ...
    protected function addEntityAssociation(EntityAssociation $association)
    {
        ...
        $association->setHydrator($this->getHydrator());
        $this->entityAssociations[$association->getAlias()] = $association;
    }

    public function addEntityColumnMap($column, $mappedColumn)
    {
        $this->entityColumnMapping[$column] = $mappedColumn;
        return $this;
    }

    public function getEntityColumnMapping()
    {
        return $this->entityColumnMapping;
    }
    ...
    {
        return array_keys($this->entityAssociations);
    }
    ...
    protected function insert($entity, HydratorInterface $hydrator = null)
    {
        ...
        foreach ($rowData as $key => $data) {
            if (array_key_exists($key, $this->entityAssociations)) {
                unset($rowData[$key]);
            }
        }

        // apply mapping
        foreach ($this->entityColumnMapping as $column => $mapping) {
            if (array_key_exists($column, $rowData)) {
                $rowData[$mapping] = $rowData[$column];
    ...
    protected function update($entity, $where = null, HydratorInterface $hydrator = null)
    {
        ...
        // apply mapping
        foreach ($this->entityColumnMapping as $column => $mapping) {
            if (array_key_exists($column, $changedData)) {
                $changedData[$mapping] = $changedData[$column];
                unset($changedData[$column]);
            }
        }
